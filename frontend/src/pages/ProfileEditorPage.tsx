...
function toForm(profile: typeof demoProfile): ProfileForm {
  return { username: profile.username, displayName: profile.displayName, headline: profile.headline, bio: profile.bio, skillsText: profile.skills.join(', '), location: profile.location };
}
...
export function ProfileEditorPage() {
  ...
  const { register, handleSubmit, reset, watch, formState: { errors } } = useForm<ProfileForm>({ resolver: zodResolver(profileSchema), defaultValues: toForm(profile) });

  useEffect(() => { if (profileQuery.data) reset(toForm(profileQuery.data)); }, [profileQuery.data, reset]);

  const preview = watch();
  const skills = (preview.skillsText ?? profile.skills.join(', ')).split(',').map((item) => item.trim()).filter(Boolean).slice(0, 6);
      ...
      <div className="split-layout page-inner">
        <div className="glass-card reveal visible">
          ...
          <form className="form editor-form" onSubmit={handleSubmit(submit)}>
            ...
            <button type="submit" className="primary-btn wide" disabled={updateProfile.isPending}>{updateProfile.isPending ? 'Сохраняем…' : 'Сохранить профиль'}</button>
          </form>
        </div>

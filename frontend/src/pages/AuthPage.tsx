...
export function AuthPage() {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<AuthForm>({
    ...
    defaultValues: { email: 'demo@profileforge.app', password: '12345678' }
  });
  ...
  const submit = async (values: AuthForm) => {
    try {
      ...
      setSession(session);
      showToast(mode === 'login' ? 'Вход выполнен. Добро пожаловать в ProfileForge.' : 'Аккаунт создан. Можно собирать профиль.');
      navigate('/dashboard');
    } catch (error) {
      ...
      <div className="split-layout page-inner">
        <div className="auth-panel glass-card reveal visible">
          ...
          <form className="form" onSubmit={handleSubmit(submit)}>
            <label>Email
              <input type="email" placeholder="hello@profileforge.app" {...register('email')} />
            ...
            <button type="submit" className="primary-btn wide" disabled={isSubmitting}>{isSubmitting ? 'Подождите…' : mode === 'login' ? 'Войти' : 'Создать аккаунт'}</button>
            <button type="button" className="ghost-btn wide" onClick={() => setMode((value) => value === 'login' ? 'register' : 'login')}>
              {mode === 'login' ? 'Создать аккаунт' : 'У меня уже есть аккаунт'}
            </button>
          </form>
          <p className="auth-note">Сейчас включён демо-режим: после подключения API Gateway форма будет работать с Auth Service.</p>
        </div>
